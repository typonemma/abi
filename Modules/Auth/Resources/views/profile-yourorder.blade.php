            ...
            @if ($page_count > 0)
                <div class="row">
                    <div class="col-sm-3 hidden-xs">
                        <a class="button size-1 style-5" href="{{ "profile-yourorder?page=" . max($page - 1, 1) }}">
                    ...
                    <div class="col-sm-6 text-center">
                        <div class="pagination-wrapper">
                            ...
                            @for ($i = $start; $i <= $end; $i++)
                                @if ($i > 0)
                                        @if ($i == $page)
                                            <a class="pagination active" href="profile-yourorder?page={{ $i }}">{{ $i }}</a>
                                        @else
                                            <a class="pagination" href="profile-yourorder?page={{ $i }}">{{ $i }}</a>
                                        @endif
                                @endif
                            @endfor
                            @if ($page + 5 <= $page_count)
                                <a class="pagination" href="profile-yourorder?page={{ $page + 5 }}">...</a>
                            @endif
                        </div>
                    </div>
                    <div class="col-sm-3 hidden-xs text-right">
                        <a class="button size-1 style-5" href="{{ "profile-yourorder?page=" . min($page + 1, $page_count)}}">
                            <span class="button-wrapper">
