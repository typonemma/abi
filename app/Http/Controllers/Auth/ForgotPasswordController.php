    ...
    if( Request::isMethod('post') && Session::token() == Request::get('_token') ){
      ...
      if($validator->fails()){
        ...
        ->withErrors( $validator );
         ...
         $get_key   = $User::where($userdata)->first();
         $data      = $User::where($userdata)->get()->count();
         $_key      = bcrypt(Request::get('user_forgot_secret_key')); 
        
         if($data>0 && isset($get_key->secret_key) && Hash::check(Request::get('user_forgot_secret_key'), $_key) && Hash::check(Request::get('user_forgot_secret_key'), $get_key->secret_key)){
           if($User :: where('email', Request::get('user_forgot_pass_email_id'))->update(['password' => bcrypt(Request::get('user_forgot_new_password'))])){
    ...
    if( Request::isMethod('post') && Session::token() == Request::get('_token') ){
      ...
      if($validator->fails()){
        ...
        ->withErrors( $validator );
         ...
         $get_key   = $User::where($userdata)->first();
         $data      = $User::where($userdata)->get()->count();
         $_key      = bcrypt(Request::get('vendor_forgot_secret_key')); 
        
         if($data>0 && isset($get_key->secret_key) && Hash::check(Request::get('vendor_forgot_secret_key'), $_key) && Hash::check(Request::get('vendor_forgot_secret_key'), $get_key->secret_key)){
           if($User :: where('email', Request::get('vendor_forgot_pass_email_id'))->update(['password' => bcrypt(Request::get('vendor_forgot_new_password'))])){
             Session::flash('success-message', Lang::get('frontend.new_password_updated'));
