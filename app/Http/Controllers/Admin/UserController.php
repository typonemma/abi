    ...
    if( Request::isMethod('post') && Session::token() == Request::get('_token') ){
      ...
      if($validator->fails()){
        ...
        if(Request::get('hf_post_type') == 'add'){
          ...
          if($check_slug === 0){
            $role_slug = string_slug_format( Request::get('user_role_name') );
          }
          elseif($check_slug > 0){
        ...
        elseif (Request::get('hf_post_type') == 'update') {
          $role_data =  array(
      ...
      if($validator->fails()){
        ...
        if(!empty($role_data)){
          ...
          if(Request::get('hf_post_type') == 'add'){
            ...
            $User->user_photo_url     =    '';
            $User->user_status        =    1;
            $User->secret_key         =    bcrypt( trim(Request::get('user_secret_key')) );
                ...
                if(Request::get('user_role') == 'vendor'){
                  ...
                  if ($Userdetails->save()) {
                    ...
                    if($email_options['vendor_new_account']['enable_disable'] == true && $env === 'production'){
                      $classGetFunction->sendCustomMail( array('source' => 'vendor_new_account', 'email' => Request::get('user_email')) );
                    }
                  }
                }
          ...
          elseif (Request::get('hf_post_type') == 'update' && $id) {
